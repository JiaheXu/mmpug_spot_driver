...
class SpotWrapper():
    ...
    def __init__(self, username, password, hostname, logger, estop_timeout=9.0, rates = {}, callbacks = {}):
        ...
        if self._robot:
            ...
            self._async_tasks = AsyncTasks(
                [self._robot_state_task, self._lease_task, self._idle_task, self._estop_monitor])
    ...
    def claim(self):
        ...
        try:
            ...
            self._logger.error("Failed to get lease: %s", e)
            return False, str(err)
    ...
    def velocity_cmd(self, v_x, v_y, v_rot, cmd_duration=0.5):
